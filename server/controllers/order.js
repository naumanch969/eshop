...
export const getUserOrder = async (req, res) => {
    try {
        const findedOrder = await Order.findOne({ user: req.params.userId })
        if (!findedOrder) return res.status(400).json({ message: 'order not exist', success: false })

        return res.status(200).json({ result: findedOrder, message: 'user order fetched successfully', success: true })
    }
    catch (error) {
        console.log(`error ----->\n`, error)
        res.status(404).json({ message: 'error in getUserOrder - controllers/order.js', error, success: false })
    }
}
